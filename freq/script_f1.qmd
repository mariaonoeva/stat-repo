...
f1_df$verb <- 0 
f1_df$verb[f1_df$condition %in% c("e", "f", "g", "h")] <- "V1 li"
f1_df$verb[f1_df$verb != "V1 li"] <- "V2" # a, b, c, d

# the same as above for the second variable 'context'
f1_df$context <- 0 
f1_df$context[f1_df$condition %in% c("a", "b", "e", "f")] <- "positive"
f1_df$context[f1_df$context != "positive"] <- "neutral" # c, d, g, h

# the same as above for the third variable 'polarity'
f1_df$polarity <- 0 
f1_df$polarity[f1_df$condition %in% c("a", "c", "g", "e")] <- "aff"
f1_df$polarity[f1_df$polarity != "aff"] <- "neg" # b, d, f, h
...
f1_df <- f1_df %>%
 mutate(condition1 = paste(condition, context, verb, polarity))

library(DescTools) # for Mode() 
f1_df$rating1 <- as.numeric(f1_df$rating1)

raw_summary <- f1_df %>%
  dplyr::group_by(polarity, verb, context) %>%
...
as_raw_html(raw_summary %>% gt(groupname_col = 'polarity', 
                               rowname_col = 'context') %>%
...
f1_df$rating1 <- as.factor(f1_df$rating1)
...
f1_main_plot <- ggplot(f1_df_relevel1, aes(fill=rating1, x=context)) + 
    geom_bar(position = "fill") +
    geom_hline(aes(yintercept=0.5), size=0.5) +
    facet_wrap(~verb+polarity) +
  # coloring
    theme_bw() +
...
polarity.labs <- c("Affirmative", "Negated")
names(polarity.labs) <- c("aff", "neg")
...
f1_main_plot1 <- ggplot(f1_df_relevel1, aes(fill = rating1, x = context)) + 
  geom_bar(position = "fill") +
  geom_hline(aes(yintercept = 0.5), size = 0.5) +
  facet_wrap(~verb + polarity, labeller = labeller(verb = verb.labs, polarity = polarity.labs)) +
  theme_bw() +
  ...
  # geom_rect(data = subset(f1_df_relevel1, verb  %in% c("V2") & indef %in% c("ni")), 
...
# as pdf 
ggsave(f1_main_plot1, file="f1_main.pdf", 
...
inter_df_f1$rating1 <- as.numeric(inter_df_f1$rating1)

# calculating interactions 
tab_inter_e1 <- summarySE(inter_df_f1, measurevar="rating1", 
                         groupvars = c("context", "verb", "polarity"))
...
# plotting 
inter_plot_f1 <- ggplot(tab_inter_e1, aes(x=context, y=rating1, 
                                    colour=polarity, group=polarity)) + 
...
# as pdf 
ggsave(inter_plot_f1, file="f1_inter.pdf", 
       width = 12, height = 12, units = "cm", device="pdf")
```
...
```{r}
cond_1 <- f1_df %>% filter(condition1 == 'a neutral V1 li ni')
...
```{r}
f1_df$rating1 <- as.numeric(f1_df$rating1)
f1_df.aov <- aov(rating1~condition1, data=f1_df)
summary(f1_df.aov)
```
...
```{r}
f1_df.aov3 <- aov(rating1~verb*indef*context, data = f1_df)
summary(f1_df.aov3)
```
...
```{r}
f1_df$rating1 <- as.numeric(f1_df$rating1) # data need to be numeric
f1_df.lm <- lm(rating1~verb*indef*context, data=f1_df)
summary(f1_df.lm)
```

```{r eval = FALSE, echo=FALSE}
library(modelsummary)
modelsummary(f1_df.lm, output = "gt", 
...
```{r eval = FALSE, echo=FALSE}
f1_df$rating1 = as.factor(f1_df$rating1)
f1_df$condition1 = as.factor(f1_df$condition1)
f1_df$context = as.factor(f1_df$context)

scat_plot <- ggplot(f1_df, aes(x=context, y=rating1, 
...
This is all cool and fun but as I mentioned a hundred times above, my data are ordinal and I finally have tot treat them as ordinal. The next and final section deals with that and presents a final model for my data -- CLMM.
...
```{r}
f1_df$rating1 = as.factor(f1_df$rating1)
e1.clm <- clm(rating1 ~ verb * indef * context, data = f1_df)
summary(e1.clm) 
```
```{r}
library(ggeffects)
...
https://marissabarlaz.github.io/portfolio/contrastcoding/ One last theory piece, I promise!

First of all, I didn't quite get why we didn't use the treatment/dummy coding for my experiment. When I learned more about what it does, I realized that it doesn't make sense to compare all my data to one reference level. It is something like having a control placebo group to which everything is compared. In this experiment, I didn't have such a group, so I need to make my comparisons to the grand mean with no baseline. But I did it for lm(), there was an intercept--condition e, `V1`, `negative` and `ni`. But all of the stuff there was just practicing, so that's fine. If one looks at their coding, they all have 0.

```{r}
f1_df$verb = as.factor(f1_df$verb)
f1_df$context = as.factor(f1_df$context)
f1_df$indef = as.factor(f1_df$indef)
contrasts(f1_df$verb)
contrasts(f1_df$context)
contrasts(f1_df$indef)
```
