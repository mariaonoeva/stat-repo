...
t-value is also clear from above (the difference between my mean and 0 in SEs). Now I also have coefficents and estimates in this table, let's unpack.
...
-   verbV2:indefnibud interaction: the negative interaction coefficient (-1.5699) tells us that the effect of `verbV2` is weaker when `indef = nibud.`
...
So far I looked whether my obtained means are different from each other. I just found out that some conditions significantly more natural then the others. But why? Well, maybe it's a chance or maybe these three independent variables that I had impacted their naturalness. Just to remind:

-   rating from 1 to 7 is my dependent variable, it is ordinal type of data (very important!)
...
Maybe one of the independent variables affected the dependent, maybe their combinations did.
