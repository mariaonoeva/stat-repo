...
Below is the most common output from ANOVA. Once again I violate perhaps all conditions that can be violated but who cares. p-value is small, so we reject H0 that all groups are equal. But it only means that at least one is different but which one? And what is the relation between other variables?
